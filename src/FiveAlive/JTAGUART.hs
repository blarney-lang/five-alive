-- Interface to Intel's memory-mapped JTAG UART component

module FiveAlive.JTAGUART
  ( AvalonJTAGUARTIns(..)
  , AvalonJTAGUARTOuts(..)
  , makeJTAGUART
  ) where
...
makeJTAGUART streamIn avlIns = do
  ...
  always do
    ...
    when (state.val .==. s_ReadWSpace) do
      let dataIn = avlIns.avl_jtaguart_readdata
      when (inv avlIns.avl_jtaguart_waitrequest) do
        state <== if slice @31 @16 dataIn .>. 0  then s_WriteData else s_Idle
  ...
  let avlOuts =
        ...
        , avl_jtaguart_address =
            if state.val .==. s_WriteData then 0 else 4
        }
